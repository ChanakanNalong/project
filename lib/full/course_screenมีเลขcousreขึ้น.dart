...
class _CourseScreenState extends State<CourseScreen> {
  ...
  Future<void> _showUpdateDialog(Course course) async {
    // Initialize controllers with current course values
            ...
            child: ListBody(
              children: <Widget>[
                TextField(
                  ...
                  readOnly: true, // Make the course code field read-only
                ),
                TextField(
            ...
            TextButton(
              ...
              onPressed: () async {
                Course updatedCourse = Course(
                  courseCode:
                      codeController.text, // courseCode should remain the same
...
// ฟังก์ชั่นดึงข้อมูล คืนค่ากลับมาเป็นข้อมูล Future ประเภท List ของ Course
Future<List<Course>> fetchCourses() async {
  // ทำการดึงข้อมูลจาก server ตาม url ที่กำหนด
  final response = await http.get(Uri.parse('http://10.5.50.97/api/course.php'));
...
Future<int> deleteCourse(Course course) async {
  final response = await http.delete(
    Uri.parse(
        'http://10.5.50.97/api/course.php?course_code=${course.courseCode}'),
...
Future<int> updateCourse(Course course) async {
  final response = await http.put(
    Uri.parse('http://10.5.50.97/api/course.php'),
    headers: <String, String>{
      'Content-Type': 'application/json; charset=UTF-8',
    },
